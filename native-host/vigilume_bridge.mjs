...
async function handleMcp(body){
  ...
  if(body.method==='server/discover')return rpcResult(id,{resultType:'complete',supportedVersions:[PROTOCOL],capabilities:{tools:{}},ttlMs:5_000,cacheScope:'private',instructions:'Quan sát và điều khiển ChatGPT Web qua Vigilume. Các action bị giới hạn bởi capability scopes trong extension.'});
...
const server=http.createServer(async(req,res)=>{
  try{
    ...
    if(req.url==='/health'&&req.method==='GET'){json(res,200,{ok:true,name:'vigilume-bridge',version:VERSION,protocol:PROTOCOL});return;}
...
server.listen(PORT,HOST,()=>console.error(`[vigilume] bridge listening on http://${HOST}:${PORT}; token: ${TOKEN_PATH}`));
server.on('error',(e)=>{console.error('[vigilume] bridge server error:',e.message);process.exitCode=1;});
