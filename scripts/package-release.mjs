...
const sourceFiles=['manifest.json','package.json','README.md','SECURITY.md','CHANGELOG.md','RELEASE_NOTES_v0.2.0.md','.gitignore',...filesUnder('_locales'),...filesUnder('src'),...filesUnder('native-host'),...filesUnder('docs'),...filesUnder('tests'),...filesUnder('scripts'),...filesUnder('.github')].filter((x)=>fs.existsSync(path.join(root,x)));
...
fs.copyFileSync(path.join(root,'RELEASE_NOTES_v0.2.0.md'),path.join(release,'RELEASE_NOTES.md'));
