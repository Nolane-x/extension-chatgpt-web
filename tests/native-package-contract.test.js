...
test('native release keeps shared task protocol, disclaimer and ESM package scope',()=>{
  assert.equal(packageJson.type,'module');
  assert.match(packager,/const nativeFiles=\['package\.json'/);
  assert.ok(packager.includes("'src/core/task-protocol.js'"));
  assert.ok(packager.includes("'DISCLAIMER.md'"));
});

test('Vigilume native bridge imports the shared task registry instead of duplicating task schemas',()=>{
  const host=fs.readFileSync('native-host/vigilume_bridge.mjs','utf8');
